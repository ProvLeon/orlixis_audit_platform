...
import { ArrowLeft, Shield, Play, Home, Copy, Search, RefreshCw } from "lucide-react"
  ...
  return (
    <main className="relative min-h-[80vh] w-full flex items-center justify-center px-6 py-8">
      ...
      <div className="w-full max-w-5xl">
        <header className="mb-6 flex items-center justify-between gap-3" role="banner" aria-label="Security not found header">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Security scan not found</h2>
              <p className="text-sm text-slate-600 dark:text-slate-300">Use the tools below to start a new scan or return to security dashboard.</p>
            </div>
          </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white/80 dark:bg-slate-900/80 backdrop-blur p-8 shadow-sm">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="mt-2 max-w-prose text-slate-600 dark:text-slate-300">
              The security scan or resource you&apos;re looking for may have been removed or is no longer accessible.
              Use the shortcuts below to continue.
            </p>
            ...
            <div className="mt-4 grid w-full grid-cols-1 gap-3 sm:grid-cols-3">
              <Button asChild className="h-11 bg-teal-600 hover:bg-teal-700 text-white">
                <Link href="/security" aria-label="Go to Security dashboard">
              ...
              <Button asChild variant="outline" className="h-11">
                <Link href="/projects" aria-label="View projects to start scans">
            ...
            <div className="mt-4 text-xs text-slate-500">
              Shortcuts: <span className="rounded border px-1.5 py-0.5">B</span> Back • <span className="rounded border px-1.5 py-0.5">S</span> Security • <span className="rounded border px-1.5 py-0.5">R</span> Reload • <span className="rounded border px-1.5 py-0.5">/</span> Search
            </div>
            <div className="mt-6 w-full rounded-lg border border-slate-200 bg-slate-50/60 p-4 text-left dark:border-slate-700 dark:bg-slate-800/60">
              <p className="text-sm text-slate-700 dark:text-slate-300">
                Tip: Security scans may be removed after completion. Check your projects to start a fresh scan.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white/80 dark:bg-slate-900/80 backdrop-blur p-6 shadow-sm">
            ...
            {recent.length === 0 ? (
              ...
              <ul className="space-y-2">
                {recent.map((p) => (
                  <li key={p.id}>
                    <Link href={`/projects/${p.id}/analysis`} className="flex items-center justify-between rounded-lg border px-3 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-800">
                      <span className="truncate">{p.name}</span>
                      <span className="text-xs text-teal-700">Analyze</span>
                    </Link>
                  </li>
                ))}
              </ul>
            ...
            <div className="mt-4">
              ...
              {detailsOpen && (
                <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-slate-600 dark:text-slate-300">
                  ...
                  <li>Use the search to find your project or start a new scan.</li>
                  <li>Security scans may be automatically cleaned up after completion.</li>
                </ul>
              )}
            </div>
          </div>
